...
static inline void f1c100s_debe_set_mode(struct fb_f1c100s_pdata_t * pdat)
{
	...
	if(~(!pdat->timing.rgb_swap)){val = 1<<2;}     // 0:rgb 1:bgr (BGR swap RGB) 
	write32((virtual_addr_t)&debe->layer0_attr1_ctrl, val | (0x09 << 8));  //TODO:
...
static struct device_t * fb_f1c100s_probe(struct driver_t * drv, struct dtnode_t * n)
{
	...
	pdat->timing.rgb_swap = dt_read_bool(n, "rgb-swap", 0);
